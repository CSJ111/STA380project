...
if (!exists("load_ecg_data")) {
  source("load_ecg_data.R")
}
if (!exists("filter_to_unique_patient")) {
  source("filter_to_unique_patient.R")
}
if (!exists("compute_age_gap")) {
  source("compute_age_gap.R")
}
if (!exists("run_permutation_test")) {
  source("run_permutation_test.R")
}
...
test_that("filter_to_unique_patient(method='latest') keeps one row per patient", {
  ...
  # the function keeps the FIRST exam within patient after sorting, which corresponds to
  # the *smallest* exam_id for each patient in this toy example.
  expect_equal(out$exam_id[out$patient_id == 1], 101)
  expect_equal(out$exam_id[out$patient_id == 2], 201)
...
test_that("run_permutation_test(stat) validates choices via match.arg()", {
  ...
  # match.arg() is case-sensitive here
